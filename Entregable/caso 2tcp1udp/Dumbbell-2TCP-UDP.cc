...
int main (int argc, char *argv[])
{
  ...
  std::string animFile = "SOR2-TP2-Dumbbell-2TCP-UDP-animation.xml" ;  
  ...
  //---onof aplication udp-------------------------
  clientHelper = ON_OFF_ApplicationHelper("ns3::UdpSocketFactory", d.GetRightIpv4Address (0));
  clientApps.Add (clientHelper.Install (d.GetLeft (2)));
  PacketSinkHelper serverHelper3 ("ns3::UdpSocketFactory",InetSocketAddress (d.GetRightIpv4Address (0), port));
  serverApps.Add (serverHelper3.Install (d.GetRight (0)));
...
//----------------------------------------
  ...
  //Generacion de pcap's. - Captura de los paquetes de cada enlace (aristas).
    if (tracing)
    {
      p2pNode.EnablePcapAll ("SOR2-Dumbbell-2TCP-UDP", false);
